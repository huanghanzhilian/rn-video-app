        ...
        <View style={styles.item}>
          ...
          >
            <View style={styles.moskBox}>
              ...
              <Icon2 name="indent-more" style={styles.albumIcon} color="#777" size={24} /> 
            </View>
    ...
    for (var i = 0, k = datas; i < k.length; i++) {
      if (k[i].id == o.parentId) { //获取一级数组
        ...
        for (var j = 0, q = k[i].data; j < q.length; j++) { //混还二级数组
          ...
          if (j == w) { //点击索引等于二级数组索引
            q[j].status = 1;
            if(k[i].key == '1_1_0'||k[i].key == '2_7_0'||k[i].key=='3_11_0'||k[i].key=='4_36_0'){
              this.setState({
                type:q[j].key
              })
            }
            ...
            if(k[i].key == '7_102_0'||k[i].key=='8_115_0'){
              this.setState({
                years:q[j].key
              })
            }
